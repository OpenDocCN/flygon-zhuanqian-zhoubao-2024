...
> 来源：[https://icdo3h8n6g.feishu.cn/docx/FnSFd77FmokYE6xp98ecVAq5nKf](https://icdo3h8n6g.feishu.cn/docx/FnSFd77FmokYE6xp98ecVAq5nKf)
...
1.  学习阶段：这个阶段，学习者从书籍或其他资源中吸收知识。

1.  教授阶段：在这里，"教授"并不是向真人讲解，而是向语言模型对齐思想（即构建 Prompt ）。这要求学习者必须理解所学内容，才能够将其转化为一个清晰、有逻辑的 Prompt。

1.  回顾和反馈：接收到语言模型的回答后，学习者会对比自己的理解和模型提供的信息，这个过程会帮助我们识别理解中的差距或不足。

1.  简化和改进：根据反馈重新调整的 Prompt，使其更加准确和简明。

## 二）连接理论与实际应用

将书籍方法论转换为 Prompt 这一过程也蕴含了经验学习圈理论：

1.  具体经验：在这一阶段，我们通过阅读书籍或其他教育资源来获得理论知识。理解这些理论的基本概念是构建有效 Prompt 的基础。

1.  观察反思：将所学的理论转化为具体的问题或情境，形成 Prompt，这个过程要求我们深入思考理论如何适用于具体的实例或问题。这不仅是对理论的重述，更是对其实际应用意义的探究。

1.  抽象概念：接下来，我们会根据语言模型对 Prompt 的回答，进一步深化对理论的理解，并知道如何更好地向模型传达抽象概念 。

1.  主动实验：Prompt 重塑了知识的留存形式和结构，通过语言模型将获得的知识和见解，应用到真实世界的问题中。
...
![](img/0ff3b404712307fbf4f7f21c3c89e400.png)

1.  动机（Motivation）：保障安全是使用安全带的主要动机。大多数人理解系安全带能显著减少交通事故中的伤害风险。

1.  能力（Ability）：车内配备有安全带，且大多数人知道如何正确使用它们。

1.  提示（Prompt）：车辆启动时的安全带警告声或视觉提示，或者驾驶前的习惯性行为，都可以作为系安全带的提示。

在缺少这些因素的情况下，人们可能不会系安全带：

1.  缺少动机：如果某人不认为系安全带很重要，或者认为短途行驶不需要系安全带，他们可能就不会系。

1.  缺少能力：如果安全带损坏或难以使用，或者乘客不知道如何正确系安全带，那么他们可能无法使用。

1.  缺少提示：在没有自动安全带提醒或没有养成系安全带习惯的情况下，即使有动机和能力，人们也可能忘记系安全带。

## 二）为什么要学《福格行为模型》：别再让动机替你背锅了
...
![](img/bd08ec209172af35ab9eb740028f9513.png)
...
![](img/7f49522f773b308dc3ac1cd933d6e9ec.png)

在探索行为选项这一步骤中，我列举了 9 个行为选项，我结合自身实际情况（外地出差、常住酒店、工作地点不完全固定等考量因素），分析这 9 个行为选项应该放在焦点地图的位置：

1.  每天进行轻度的散步：低影响，中等难度

1.  定期进行腰部拉伸运动：高影响，中低难度

1.  时刻保持良好的坐姿：高影响，高难度（难以时刻做到）

1.  使用支撑腰部的坐垫或椅子：中影响，高难度（找到合适的坐垫和椅子比较困难）

1.  控制饮食，维持健康的体重水平：中影响，高难度（控制饮食并不简单）

1.  定期热敷腰部：中影响，高难度（所住地点环境温度低，热敷效果打折）

1.  在办公或长时间坐着时定期站起来活动：中高影响，低难度；可以配合第二点一起实施

1.  接受专业的按摩治疗：高影响，高难度（需要金钱和时间）

1.  在睡觉时使用适合的枕头和床垫：低影响，低难度（酒店的床垫和枕头就很不错）

根据上述分析结论，绘制焦点地图：

![](img/893deb5303a5c89bb20bd2482f7aedd3.png)
...
![](img/1582d0363dc31d538157d19be7767c3e.png)

回到腰部拉伸运动，用能力因素分析：

1.  时间：腰部拉伸要高频率，但一般来说只需持续 1 分钟
...
1.  日程：不会与我的日程冲突，我在任何时间任何地点都可以做这个动作

探索让它更容易做到的三种方式：

1.  提升技能：在 b 站搜教学视频即可，动作本身并不难

1.  获取工具和资源：不需要借助外部工具资源

1.  让行为变得微小：

1.  入门步骤：双手扶着腰

1.  缩小规模：每次考虑只做 1 个拉伸动作
...
![](img/e7487ccd705ca66ff55a3bae4a83cd87.png)

我为腰部拉伸运动设计的锚点的最后动作是：想上厕所起身并离开椅子。分析这个锚点是否匹配锚点选择的三要素：

1.  匹配物理位置：上厕所前我基本都会坐在椅子上，物理位置匹配

1.  匹配频率：我本人一天上厕所的频率很高（这与我的另一个习惯：每天喝 2.5L 水以上的有关），所以与腰部拉伸的频率十分契合。

1.  匹配主题/目的：上厕所是休息的契机，与腰部拉伸运动的主题一致。
...
*   锚点的最后动作：想上厕所起身并离开椅子时

*   新行为的入门步骤：双手扶着腰

*   庆祝：在做完后，深吸一口气并微笑。

*   完整的行为设计：在想上厕所起身并离开椅子时，我将双手扶着腰开始做 3 - 5 个腰部伸展运动，在做完后，深吸一口气并微笑。
...
## Initialization
以“你好，我是你的福格行为模型专家，我将通过福格行为模型帮助您做到他们本来就想做到的事情并感受成功！请您描述一下您想要做到的事习惯或愿望，如'我想要每天运动 30 分钟'、'我想要身体更健康'，以便我可以提供更好的解决方案。”为开场白和用户对话，必须遵循，按照<workflow>的流程开始工作。</workflow> 
```
...
## Initialization
以“你好，我是你的福格行为模型专家，我将帮助您终止不良习惯并感受到成功！请描述您想要终止的习惯，如'我想要戒烟'或'我想减少社交媒体使用时间'，以便我为您提供更好的解决方案。”作为开场白与用户对话，遵循并按照<workflow>流程。</workflow> 
```
...
1.  理解 “Read in Prompt out” 的重要意义，提高你动手写提示词的动机。

1.  被安利一了本叫《福格行为模型》的行为科学类书籍，知道了做事不能仅凭意志力，需要进行效的行为设计。

1.  通过“缓解腰痛疼痛症状”这一案例，掌握行为设计的全流程，并能将其应用于生活实践。

1.  获得了帮助你创建良好习惯、终止坏习惯的实用 GPTS 和它们的原始的 Prompt ，了解调试 Prompt 过程中可能遇到的问题和有效的解决方案。

1.  从上述行为设计案例中，吸收可借鉴的行为设计技巧。
