# Cursor+AI：2天打造一个高效公众号矩阵系统，实现内容创作与发布自动化

> 来源：[https://ia0969wpr2.feishu.cn/docx/K3BidxkyHodPfMxlzEOc1W64nbe](https://ia0969wpr2.feishu.cn/docx/K3BidxkyHodPfMxlzEOc1W64nbe)
...
![](img/86324c76ba3b7d72bc94a7ef36abb653.png)

素材管理：这里我建了一个专属于自己的本地素材库，可以管理所有原始采集数据经过改写后的文本，可以手动添加或者导入新素材，也可以查看修改前后的对比并进行发布。

![](img/ddce4741a53e8e3b1cc111f43a026aa7.png)

发布的时候可以选择多个公众号进行发布。

![](img/2b4a60967606a07c092d555c9e99d26d.png)

![](img/f4c7290351b19a5cd29032b00f587dee.png)

发布任务管理：这里所有待发布的任务都会放到这里，可以查看发布详情等

![](img/e4d7a61c5fcec7f4f253590972e50ae5.png)

![](img/871008d675885f13d9cea942f27bf51d.png)
...
![](img/ff1cd050ca13b2c5652e0ff9b077806e.png)

![](img/fd1770b2a0cc4b2a5a20ef90bede75af.png)

多环境管理：这里可以设置多个浏览器，只需要确保每个浏览器绑定一个账号，这样我们就可以实现矩阵化管理了

![](img/4906670323f6d1f8927ee26b5c2a63ea.png)

![](img/e2c38a19154d07c40c072504072e6758.png)

这篇文章中有什么：

1.  包含了我在开发过程中的一些关键提示词，整个项目的开发大概花了两天时间，包括前端页面、后端逻辑和自动化发布、收益获取与可视化等后端逻辑。除了极少部分的文案和代码是tab补全的以外，其他所有的代码生成、样式布局修改、排错等都是通过composer 或者 windsurf进行的。在这个过程中涉及到的主要提示词和思路罗列在了文章中，当然一些排错的交互没有加上，因为现在的AI模型能力下有问题直接问cursor，开发过程中遇到的错误也不尽相同，逢山开路遇水搭桥就可以。

1.  另外这里面还包括一些谷歌和GitHub的搜索技巧。即便在AI时代，我们也不必所有代码都从零开始写。通过借鉴和学习优秀的开源项目，既可以节省开发时间，又能提升代码质量。因此，我也把这个过程中代码搜索和整合的思路整理在了文档中。
...
![](img/a1025e0a64a31cfe4cdb2cd980cf19a8.png)
...
![](img/0476d47c243bcea55e081a98b4a5ff8e.png)

![](img/1143ad0bdb024679e2ce61ca473f3c62.png)

![](img/a514677f943b3931aef199e5989e00a6.png)

等待执行完之后我们直接运行，如下所示，我们已经成功访问了本地的项目。从构思到设计功能到生成提示词花了半小时，代码生成到调试报错运行只花了不到五分钟，一个基本的框架就已经搭建完成了，这个效率还是很快的吧？

![](img/ba493ec5252de0af182dc4ad965073c1.png)
...
![](img/d7a24b30fe1d76c5392b2b45de88fa64.png)

![](img/50331613ecdd2b9a73991e39749598e2.png)

我们仔细思考一下这个流程图的主要逻辑，发现是没有问题的，那我们就按照这个逻辑生成提示词

![](img/fd2c35f19caea60d9e3bd9e23ba1b67d.png)
...
![](img/8a40272252202d8a36c63f18a55cceed.png)

![](img/827973dd6de804797ffcd695911a228e.png)
...
![](img/05d11c5b1678c51e7d119bb270683b84.png)
...
![](img/956f93b64c8734e1c57dd05b0f0c7e6d.png)

我们直接在google 和 github 搜索指定关键词，看下是否有有人实现过。我们双关键词搜索，如下：

![](img/896ef6a70e6ba354c9514c26c6af09ea.png)

![](img/b190a69091420cf5cc3b65aec4c8e124.png)

太棒了，我们在google 和 github 至少找到了五六个完整的关于比特浏览器的SDK，要啥自行车啊？我们随便拿来一个用。

![](img/40d37561e70fb11ebafeae544dc90025.png)

![](img/bd4ab5977f9d653ceb73d8d47d225869.png)
...
![](img/d699da19374c5d6c23cee7fa22fd284e.png)
...
![](img/e902f1133a42ce1bee7a837323dbe6e2.png)
...
![](img/0aad3274af44636b1fc36c54193eee35.png)
...
1.  通用简单、门槛比较低的影刀RPA方式。这种方式当然可行，但是影刀不支持社区版的免费调度，也就是说社区版我们是没有办法使用API调度的。另外，影刀在管理矩阵时的效率也很慢。

1.  使用Playwright。Playwright相比影刀的优点也很明确，就是影刀的缺点就是他的优点，而且我们还可以使用AI帮助我们生成代码，借助Cursor进行元素定位，大部分自动化流程我们一小时之内就可以实现，而影刀手动拖拽实现自动化流程，调试好可能至少需要一到两小时，而且换台机器可能还会有兼容性问题。playwright可以无缝衔接指纹浏览器，同时也可以更加快速方便地横向扩展，我们可以开启十个、二十个，甚至更多的浏览器来帮助我们完成矩阵化运营。综合来看不管是写代码的效率和执行的效率playwright都降维打击影刀。

那比对下来我们就采用playwright方案了

从整个流程来看，我们需要两个节点帮助我们执行任务：

1.  一个可靠的定时任务调度器，它负责按照预设的时间表执行执行指令。

1.  一个灵活的手动触发机制，作为定时发布的补充，让我们可以随时掌控发布节奏
...
1.  直接建一个README，写上目前的这些流程

1.  安装插件，把所有的元素定位都拷贝下来

1.  写入Cursor，或通过逐步提问的方式，让Cursor帮助我们完成这个自动化流程
...
![](img/61581d6a7e333d91a2f1efd863a1684f.png)

这里按照正常的发布流程逐个采集元素位置发给cursor即可，就不逐步写保姆级教程了，比如我们需要首先创建草稿，选中图文元素：

![](img/9e51ec3a0354b00f09165199d72d83fe.png)

![](img/a5ee12618b144f4335e09c4377e3d879.png)

![](img/e8e4eb793bf1ff1750d5cca90e5730a0.png)
